...
Terraform manages **all** long-lived resources: Cloudflare (D1 / KV / Queues / Bindings /
Cron), Backblaze B2 (data buckets + worker application key), and Neo4j AuraDB instance.

```bash
# 1. Export Cloudflare and Backblaze B2 master credentials
export CLOUDFLARE_API_TOKEN=<your-token>
export TF_VAR_account_id=<cf-account-id>
export AWS_ACCESS_KEY_ID=<B2_STATE_KEY_ID>      # state-bucket key (or B2 admin key)
export AWS_SECRET_ACCESS_KEY=<B2_STATE_KEY>
export B2_APPLICATION_KEY_ID=<B2_MASTER_KEY_ID>  # B2 admin key (B2 provider)
export B2_APPLICATION_KEY=<B2_MASTER_KEY>
export AURA_CLIENT_ID=<neo4j-aura-client-id>
export AURA_CLIENT_SECRET=<neo4j-aura-client-secret>

# 2. Terraform provision everything
...
After `apply`, Terraform outputs include a **bucket-scoped B2 worker key** (least-privilege)
and **Neo4j connection details**. When run via `terraform.yml`, these are automatically
pushed to GitHub Secrets/Variables (see §Deployment for the credential flow).
...
| `NEO4J_URI` | Graph / Cleanup | `neo4j+s://...` connection URL (wrangler.toml `[vars]`, patched from Terraform state by deploy.yml) |
| `NEO4J_USERNAME` | Graph / Cleanup | Neo4j username (wrangler.toml `[vars]`, patched from Terraform state by deploy.yml) |
| `NEO4J_DATABASE` | Graph / Cleanup | Neo4j database name (wrangler.toml `[vars]`) |
| `NEO4J_PASSWORD` | Graph / Cleanup (Secret) | Neo4j password (read from Terraform state and uploaded by deploy.yml) |
| `JWT_SECRET` | Gateway / User (Secret) | ≥32 char high-entropy random string; 5-tier strength validation |
| `B2_MASTER_KEY_ID` / `B2_MASTER_KEY` | B2 provider | B2 admin key — manually configured; used only by Terraform (prefer a non-master key with bucket/key write caps) |
| `B2_STATE_KEY_ID` / `B2_STATE_KEY` | TF state backend (optional) | Key scoped to the state bucket; falls back to the admin key |
| `B2_KEY_ID` / `B2_KEY` | Ingestion / Cleanup (Secret) | B2 **worker** key — auto-generated by Terraform (bucket-scoped, least-privilege), uploaded as Worker Secrets by deploy.yml (read from Terraform state) |
| `GOOGLE_API_KEY` / `GROQ_API_KEY` | AI (Secret, optional) | LLM provider keys; Workers AI needs no key |
| `EMAIL_API_KEY` | User (Secret, optional) | Resend API key for credential notification emails; if unset, credentials are returned via API response instead |
...
| Secret | `B2_MASTER_KEY_ID` / `B2_MASTER_KEY` | B2 admin key — Terraform B2 provider |
| Secret | `B2_STATE_KEY_ID` / `B2_STATE_KEY` | (optional) state-bucket-only key for the Terraform S3 backend |
| Secret | `B2_STATE_READ_KEY_ID` / `B2_STATE_READ_KEY` | (recommended) read-only state key — deploy.yml reads Terraform outputs |
| Secret | `NEO4J_AURA_CLIENT_ID` / `NEO4J_AURA_CLIENT_SECRET` | Neo4j Aura API auth (Terraform creates the instance) |
| Secret | `JWT_SECRET` | Gateway + User |
| Secret | `EMAIL_API_KEY` | User (optional — Resend for credential emails) |
| Secret | `GOOGLE_API_KEY` / `GROQ_API_KEY` | AI (optional) |

**Read from Terraform state by `deploy.yml` and uploaded as Cloudflare Worker Secrets (never to GitHub):**

| Worker Secret | Workers | Source |
| ------------- | ------- | ------ |
| `B2_KEY_ID` / `B2_KEY` | ingestion + cleanup | Terraform `b2_application_key.worker` (bucket-scoped, least-privilege) |
| `NEO4J_PASSWORD` | graph + cleanup | Terraform `neo4jaura_instance.neo4j.password` |

`NEO4J_URI` / `NEO4J_USERNAME` are patched into wrangler.toml `[vars]` from Terraform state at deploy time; `NEO4J_DATABASE` is a static `[vars]` entry. Bootstrap is two steps: `terraform.yml` apply → `deploy.yml`.
...
**Credential flow:** `terraform apply` → creates B2 buckets + worker key + Neo4j instance → pushes outputs to GitHub Secrets/Variables via `gh` CLI → `deploy.yml` reads them and injects into Workers.
...
| Neo4j connection fails | Disallowed `NEO4J_URI` protocol / AuraDB domain validation fails | Use protocols accepted by `validators.neo4jUrl()`; confirm allow-listed domains |

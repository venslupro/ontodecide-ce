...
export AWS_ACCESS_KEY_ID=<B2_MASTER_KEY_ID>   # B2 master key ID
export AWS_SECRET_ACCESS_KEY=<B2_MASTER_KEY>  # B2 master key secret
...
| `NEO4J_URI` | Graph / Cleanup | `neo4j+s://...` connection URL (auto-set from Terraform output) |
| `NEO4J_USERNAME` | Graph / Cleanup | Neo4j username (auto-set; AuraDB default is the instance ID) |
| `NEO4J_DATABASE` | Graph / Cleanup | Neo4j database name (auto-set; AuraDB default is `neo4j`) |
| `NEO4J_PASSWORD` | Graph / Cleanup (Secret) | Neo4j password (auto-pushed by Terraform apply) |
| `JWT_SECRET` | Gateway / User (Secret) | ≥32 char high-entropy random string; 5-tier strength validation |
| `B2_MASTER_KEY_ID` / `B2_MASTER_KEY` | TF state backend + B2 provider | B2 **master** key — manually configured; used only by Terraform |
| `B2_KEY_ID` / `B2_KEY` | Ingestion / Cleanup (Secret) | B2 **worker** key — auto-generated by Terraform (bucket-scoped, least-privilege), exposed to workers as `B2_KEY_ID`/`B2_KEY` env vars |
...
| Secret | `B2_MASTER_KEY_ID` / `B2_MASTER_KEY` | B2 **master** key — Terraform S3 backend + B2 provider |
| Secret | `NEO4J_AURA_CLIENT_ID` / `NEO4J_AURA_CLIENT_SECRET` | Neo4j Aura API auth (Terraform creates the instance) |
| Secret | `REPO_TOKEN` | GitHub PAT (`repo` scope) — pushes auto-generated creds to Secrets/Variables after apply |
...
**Auto-created by `terraform.yml` after apply (do NOT set manually):**

| Kind | Name | Source |
| ---- | ---- | ------ |
| Secret | `B2_WORKER_KEY_ID` / `B2_WORKER_KEY` | Terraform `b2_application_key.worker` (bucket-scoped, least-privilege) |
| Secret | `NEO4J_PASSWORD` | Terraform `neo4jaura_instance.neo4j.password` |
| Variable | `NEO4J_URI` | Terraform `neo4jaura_instance.neo4j.connection_url` |
| Variable | `NEO4J_USERNAME` | Terraform `neo4jaura_instance.neo4j.username` |
| Variable | `NEO4J_DATABASE` | Fixed value `neo4j` (AuraDB default) |
